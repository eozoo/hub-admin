 ...
 * @author shanhuiming
 */
public interface HubUserRepository extends HubUserRepositoryFacade, IService<HubUser> {
     ...
     * 批量保存用户角色
     */
    void saveUserRolesBatch(List<HubUserRole> list);

    /**
     * 批量保存用户部门岗位
     */
    void saveUserDeptsBatch(List<HubUserDept> list);

    /**
     * 批量保存用户上下级关系
     */
    void saveUserDiagramBatch(List<HubUserDiagram> list);

    /**
     * 批量导入用户
     */
    void batchInsert(List<HubUser> list, boolean overwrite);
     ...
     */
    void updateLdapByCode(HubUser hubUser);

    /**
     * 更新Gitlab用户信息（按userCode）
     */
    void updateGitlabByCode(HubUser hubUser);
}
