...
public class SysConfigBizImpl implements SysConfigBiz {
    ...
    @Override
    public void editConfig(SysConfig config) {
        HttpAsserts.notNull(config.getConfigId(), BAD_REQUEST, "{admin.config.id.null}");

        SysConfig pre = configRepository.queryById(config.getTenantId(), config.getConfigId());
    ...
    public void deleteConfigs(String tenantId, List<Integer> configIds) {
        List<SysConfig> list = configRepository.listByIds(configIds);
