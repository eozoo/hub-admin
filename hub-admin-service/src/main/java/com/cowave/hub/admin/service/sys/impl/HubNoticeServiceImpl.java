...
public class HubNoticeServiceImpl implements HubNoticeService {
    ...
    public Response.Page<NoticeVo> list(String tenantId, NoticeQuery query) {
        Page<HubNotice> page = noticeRepositoryFacade.queryPage(tenantId, query);
        Map<String, String> codeNameMap = resolveCodeNameMap(page.getRecords(), HubNotice::getCreateBy);
    ...
    public NoticeVo info(String tenantId, Long noticeId) {
        HubNotice notice = noticeRepositoryFacade.queryById(tenantId, noticeId);
    ...
    public Response.Page<NoticeUserVo> queryNoticeReaders(String tenantId, Long noticeId) {
        Page<HubNoticeUser> page = noticeRepositoryFacade.queryUserPageByNoticeId(noticeId);
        Map<String, String> codeNameMap = resolveCodeNameMap(page.getRecords(), HubNoticeUser::getUserCode);
    ...
    public List<NoticeCommentVo> queryCommentList(Long noticeId) {
        List<HubNoticeComment> comments = noticeRepositoryFacade.listComments(noticeId);
        if (CollectionUtils.isEmpty(comments)) {
            return List.of();
        }

        String userCode = Access.userCode();
        Set<Long> commentIds = comments.stream().map(HubNoticeComment::getId).collect(Collectors.toSet());
        Set<Long> likedIds = noticeRepositoryFacade.likedTargets(2, commentIds, userCode).stream()
                .map(HubNoticeLike::getTargetId).collect(Collectors.toSet());
    ...
    @Override
    public List<HubNoticeLike> queryLikers(Integer targetType, Long targetId) {
