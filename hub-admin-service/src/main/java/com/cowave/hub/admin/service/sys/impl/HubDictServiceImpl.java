...
import com.cowave.hub.admin.domain.sys.biz.HubDictBiz;
...
public class HubDictServiceImpl implements HubDictService {
    ...
    private final HubDictBiz dictBiz;

    private final HubDictRepositoryFacade dictRepositoryFacade;
    ...
    public HubDict queryByCode(String dictCode) {
        HubDict dict = dictRepositoryFacade.queryByCode(dictCode);
        if (dict == null) {
            return null;
        }
    ...
    @Override
    public List<HubDict> queryListByType(String typeCode) {
        List<HubDict> list = dictRepositoryFacade.listByType(typeCode);
        if (list.isEmpty()) {
            return list;
        }
    ...
    @Override
    public List<HubDict> queryListByGroup(String groupCode) {
        List<HubDict> list = dictRepositoryFacade.listByGroup(groupCode);
        if (list.isEmpty()) {
            return list;
        }
        for (HubDict dict : list) {
