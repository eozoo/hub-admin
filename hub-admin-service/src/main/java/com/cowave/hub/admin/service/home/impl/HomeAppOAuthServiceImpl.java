...
@Service
public class HomeAppOAuthServiceImpl implements HomeAppOAuthService {
    ...
    public OAuth2CodeVo getClientCode(OAuth2CodeReq codeReq) {
        // 验证应用id
        HubApp oauthApp = hubAppRepositoryFacade.queryByClientId(codeReq.getClientId());
    ...
    public AccessUserDetails getClientToken(OAuth2TokenReq tokenReq) {
        ...
        // 验证应用id
        HubApp oauthApp = hubAppRepositoryFacade.queryByClientId(tokenReq.getClientId());
        HttpAsserts.notNull(oauthApp, BAD_REQUEST, "{admin.oauth.name.not.exist}");
