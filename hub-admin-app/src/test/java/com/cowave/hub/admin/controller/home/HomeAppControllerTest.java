 ...
 * @author shanhuiming
 */
public class HomeAppControllerTest extends SpringTest {
     ...
     * post /api/v1/home/app/authorize/code
     * get /api/v1/home/app/authorize/redirect/{code}
     * post /api/v1/home/app/authorize/token
     * get /api/v1/home/app/authorize/refresh
     * delete /api/v1/home/app/authorize/token
     * get /api/v1/home/app/authorize/refresh (fail)
     * delete /api/v1/home/app/{id}
     * delete /api/v1/auth/logout
     */
    @Test
    public void homeApp() throws Exception {
        // 登录
        String body = """
                {
                "tenantId" : "cowave",
        ...
        body = """
                ...
                """;
        mvcResult = mockPost("/api/v1/home/app", body, accessToken);
        ...
        // 获取授权应用列表
        mockGet("/api/v1/home/app?clientName=测试", accessToken);
        // 获取授权应用选项
        ...
        body = String.format("""
                ...
                """, appId);
        mockPost("/api/v1/home/app/role", body, accessToken);
        // 获取普通角色授权应用
        mvcResult = mockGet("/api/v1/home/app/role/3", accessToken);
        List<Integer> roleAppIds = readData(mvcResult, "/data", new TypeReference<>() {});
        Assertions.assertTrue(roleAppIds.contains(appId));
        // 获取授权应用的菜单列表
        mockGet("/api/v1/home/app/menu/" + appId, accessToken);
        ...
        mvcResult = mockPost("/api/v1/home/app/authorize/code", body, accessToken);
        String code = readString(mvcResult, "/data/code");
        Assertions.assertNotNull(code);
        // ② 应用回调（302跳转回客户端应用，携带code和state）
        MvcResult redirectResult = mockGet("/api/v1/home/app/authorize/redirect/" + code, null, 302);
        ...
        mvcResult = mockPost("/api/v1/home/app/authorize/token", body);
        ...
        // ④ 刷新令牌
        mvcResult = mockGet("/api/v1/home/app/authorize/refresh?refreshToken=" + refreshToken);
        ...
        mockDelete("/api/v1/home/app/authorize/token?type=" + authType + "&account=" + username + "&id=" + oauthId, accessToken);
        // ⑥ 刷新令牌401
        mockGet("/api/v1/home/app/authorize/refresh?refreshToken=" + refreshToken, null, 401);
        // ⑦ 删除授权应用
        mockDelete("/api/v1/home/app/" + appId, accessToken);
        // ⑧ 退出登录
